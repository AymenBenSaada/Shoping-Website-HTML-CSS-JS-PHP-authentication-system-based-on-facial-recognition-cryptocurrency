...
<body>
		...
		<?php
				session_start();
			require 'db.php';
			$req = $conn->prepare('SELECT `id`, `name`,`gender`, `categoryId`, `price`, `inPromotion`, `percentage`, `amount`, `availability` FROM `Product` WHERE `archive`=0 and categoryId=? and gender="w" ORDER by categoryId ') or die ("erreur in preparing select request");

			$req->bind_param('i',$_GET['categ']);
