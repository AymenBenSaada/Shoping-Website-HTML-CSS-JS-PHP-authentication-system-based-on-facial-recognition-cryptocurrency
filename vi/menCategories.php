...
<body>
		...
		<?php
				session_start();
			require 'db.php';
			$req = $conn->prepare('SELECT `id`, `name`,`gender`, `categoryId`, `price`, `inPromotion`, `percentage`, `amount`, `availability` FROM `Product` WHERE `archive`=0 and categoryId=? and gender="m" ') or die ("erreur in preparing select request");
