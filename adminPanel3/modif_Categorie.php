...
<?php
			require "db.php";
			$req = $conn->prepare('SELECT `id`, `name`, `description` FROM `Category` WHERE id=?') or die ("erreur in preparing select request");

			//binding his email and his password
			$req->bind_param("i",$_GET['id']) or die("binding erreur");
			$req->execute() or die ("executing select request error");
			$req->bind_result($id,$name,$description);
			while ($req->fetch()) 
   			 	{	echo "string";

   			 		echo "<input type='text' class='form-control'  placeholder='Category name' name='categoryName' value=".$name."><br>";
   			 		echo "<textarea name='description' class='form-control' value=".$description.">".$description."</textarea>";
				}
...

{
	...
	if($_POST['description']!==$description or $_POST['categoryName']!==$name)
		{
			
			try
			{
				$req1 = $conn->prepare('UPDATE `Category` SET `name`=?,`description`=? WHERE id=?') or die ("erreur in preparing insert request");

				$req1->bind_param("ssi",$_POST['categoryName'],$_POST['description'],$id) or die("binding erreur");
				$status=$req1->execute();
				if ($status === false) {
